...
import vn.attendance.lib.Utils;
import vn.attendance.lib.enumeration.EM_EVENT_IVS_TYPE;
import vn.attendance.lib.structure.*;
                ...
                if(msg.nImageInfoNum > 0){
                    ...
                    for(int i = 0; i < msg.nImageInfoNum; i ++){
                        ...
                        if (pstuImageInfo != null && pstuImageInfo[i].nLength > 0) {
                            ...
                            String bigPicture = picturePath + "\\" + System.currentTimeMillis() + "related_MOTORPARKING_pstuImageInfo_"+ i + ".jpg";
...
//             	PrintStruct.print(msg);
            ...
            case EVENT_IVS_PSRISEDETECTION: ///<   囚犯起身事件
            {
                NetSDKLib.DEV_EVENT_PSRISEDETECTION_INFO msg = new NetSDKLib.DEV_EVENT_PSRISEDETECTION_INFO();
                ToolKits.GetPointerData(pAlarmInfo, msg);
                String Picture = picturePath + "\\" + System.currentTimeMillis() + ".jpg";
                ToolKits.savePicture(pBuffer, 0, dwBufSize, Picture);
                System.out.println(" 囚犯起身事件  时间(UTC)：" + msg.UTC + " 通道号:" + msg.nChannelID);
                break;
            }
            ...
            case EVENT_IVS_FACEDETECT://人脸检测事件
            {
                ...
                AvataUtils.convertImageToBase64String(picture);
